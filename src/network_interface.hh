...
private:
  ...
  // ip->mac lookup table
  std::unordered_map<uint32_t, MacAddress> ARP_table{}; 

  // ip->queue of datagrams waiting for MAC address to be sent to this ip address from ARP request
  std::unordered_map<uint32_t, std::queue <InternetDatagram>> wait_mac_queue{};

  // ip->last time ARP request has been made for this ip
  std::unordered_map<uint32_t, size_t> last_ARP_time{};

  // queue of ready-to-be-sent frames
